#include <stdio.h>
 ...
 void receberdados() {
  ...
  void imprimirdados() {
   ...
   printf("Quantidade de Pontos Turísticos da primeira carta: %d\n", pontosturisticos1);
   ...
   printf("Quantidade de Pontos Turísticos da segunda carta: %d\n", pontosturisticos2);
  ...
  void comparar() {
    ...
    float vencedorpopulacao = populacao1 > populacao2;
    printf("População: Carta %.0f venceu (%.0f)\n", 2 - vencedorpopulacao, vencedorpopulacao);

    float vencedorarea = area1 > area2;
    printf("Área: Carta %.0f venceu (%.0f)\n", 2 - vencedorarea, vencedorarea);

    float vencedorpib = pib1 > pib2;
    printf("PIB: Carta %.0f venceu (%.0f)\n", 2 - vencedorpib, vencedorpib);

    float vencedorpib_percapito = pib_percapito1 > pib_percapito2;
    printf("PIB per capita: Carta %.0f venceu (%.0f)\n", 2 - vencedorpib_percapito, vencedorpib_percapito);

    float vencedordensidade_populacional = densidade_populacional1 > densidade_populacional2;
    printf("Densidade populacional: Carta %.0f venceu (%.0f)\n", 2 - vencedordensidade_populacional, vencedordensidade_populacional);

    int vencedorpontosturisticos = pontosturisticos1 > pontosturisticos2;
    printf("Pontos Turísticos: Carta %d venceu (%d)\n", 2 - vencedorpontosturisticos, vencedorpontosturisticos);
    

    float carta1 = pib1 + pontosturisticos1 + populacao1 + area1 + pib_percapito1 + (1 / densidade_populacional1);
    float carta2 = pib2 + pontosturisticos2 + populacao2 + area2 + pib_percapito2 + (1 / densidade_populacional2);
    int vencedorsuper = carta1 > carta2;
    printf("Super Poder: Carta %d venceu (%d)", 2 - vencedorsuper, vencedorsuper);
  }
